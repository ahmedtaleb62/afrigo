...
/// `initState` (a resumed session needs this too; a driver's app in this
/// same suite once went online with no live channel ever subscribed
/// because that only happened on the login path — this mirrors the fix).
///
/// Foreground message display is intentionally not handled here: every
/// screen that matters (order tracking, incoming ride/food status) already
/// gets live updates via Realtime while the app is open. Push exists
/// specifically for when the app is backgrounded/killed and Realtime can't
/// reach it — the OS displays the system notification for that case with
/// no extra code needed on the `notification` payload FCM sends.
