-- Bug: get_order_counterpart's `ride`/`delivery_request` branches only
-- resolved when the CALLER is the client (`where ... and client_id = v_caller`).
-- When the driver/livreur called it (e.g. the taxi app's
-- `_fetchClientContact`, right after accepting a ride), the predicate never
-- matched, the function silently returned null, and the driver never saw
-- the client's name or phone for any ride, ever. Make both branches
-- resolve in either direction: caller is the client -> return the
-- driver's info; caller is the driver -> return the client's info (name/
-- phone only, no vehicle fields, since the client has no vehicle).
